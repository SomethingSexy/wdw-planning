...
const ResortItem = ({ resort }) => {
    ...
    return (react_1.default.createElement(LocationItem_1.default, { description: item.description, detailPath: `/resorts/${item.id}`, key: item.id, icon: free_brands_svg_icons_1.faFortAwesome, id: item.id, meta: meta, name: item.name, tags: item.areas, type: `Resort - ${item.tier}` }));
};
